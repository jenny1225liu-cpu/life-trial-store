...
ON CONFLICT (id) DO UPDATE SET
  ...
  trend = EXCLUDED.trend, outlook = EXCLUDED.outlook, hot_skills = EXCLUDED.hot_skills,
  avg_salary_range = EXCLUDED.avg_salary_range, risk_level = EXCLUDED.risk_level;
...
ON CONFLICT (id) DO UPDATE SET
  ...
  description = EXCLUDED.description, living_cost = EXCLUDED.living_cost,
  avg_rent = EXCLUDED.avg_rent, vibe = EXCLUDED.vibe, population = EXCLUDED.population;

-- 3. 岗位数据（核心岗位，关联行业+试玩副本）
...
('tea artisan', 'tea', '茶艺师/制茶人', '🍵', '守护千年茶文化，让传统在当代绽放新生命', '{"传统制茶工艺传承", "茶文化体验活动设计", "品牌内容创作", "客户关系维护"}', '{"制茶工艺", "茶文化知识", "审美能力"}', '{"自媒体运营", "品牌运营", "直播带货"}', '不限(师徒制)', '1-5年', 75, 50, 35, 80, 35, 60, 25, '{"tea_grower_anxi"}')
ON CONFLICT (id) DO UPDATE SET
...
('product_manager', 'ai_replacement', '中', 'AI可辅助需求分析和文档撰写，但产品决策仍需人类判断力', '培养商业敏感度和跨领域洞察力', 0.3, 40),
('product_manager', 'market_cycle', '中', '互联网行业周期波动，裁员风险存在', '建立复合技能矩阵，不绑定单一赛道', 0.4, 50),
('ux_designer', 'ai_replacement', '高', 'AI设计工具可替代大量基础设计执行工作', '从执行者升级为创意策略者，强化个人风格', 0.6, 60),
('ai_engineer', 'market_cycle', '低', 'AI人才需求持续旺盛，短期不会降温', '持续学习前沿技术，保持竞争力', 0.15, 30),
('ai_engineer', 'health', '中', '长期高压研发可能影响身心健康', '建立运动习惯，注意劳逸结合', 0.5, 55),
('software_engineer', 'ai_replacement', '中', 'AI辅助编程普及，初级CRUD工程师最危险', '从写代码升级为设计系统', 0.4, 50),
('lawyer', 'ai_replacement', '低', '法律分析可被AI辅助，但庭审和谈判不可替代', '聚焦高价值的人情洞察和复杂案件', 0.2, 30),
('lawyer', 'market_cycle', '中', '经济下行期法律需求反而可能增加', '建立多元化客户来源', 0.3, 35),
('livestream_host', 'market_cycle', '高', '直播行业竞争激烈，流量获取成本持续上升', '从流量思维转向品牌思维，建立私域', 0.6, 70),
('livestream_host', 'health', '中', '长期高强度直播影响嗓子和作息', '注意嗓音保护，建立规律作息', 0.5, 45),
('homestay_owner', 'market_cycle', '高', '民宿行业受旅游淡旺季影响大，收入不稳定', '发展民宿+模式，淡季做内容/活动', 0.7, 65),
('kol_creator', 'ai_replacement', '中', 'AI内容工具让创作门槛降低，竞争加剧', '强化个人IP的独特性和真实感', 0.35, 45),
('ecommerce_operator', 'market_cycle', '中', '电商竞争白热化，平台规则变化快', '掌握多平台运营能力，关注新兴渠道', 0.45, 50),
('film_producer', 'market_cycle', '高', '影视行业周期性明显，项目制收入不稳定', '储备现金流，拓展短剧等轻量级内容', 0.65, 60),
('flight_attendant', 'health', '中', '长期飞行影响生物钟和身体健康', '注意休息规律，加强锻炼', 0.55, 40),
('tea_artisan', 'ai_replacement', '低', '手工制茶工艺难以被AI完全替代', '持续精进传统工艺，结合现代营销', 0.1, 15);
